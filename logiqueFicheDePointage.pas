...
{ Quand nous recevons un boolean de la BDD, il est représenter sous forme de 1 et 0
...
{ L'heure renvoyé par la BDD est sous cette forme : "00:00:00", alors ce qu'on fait,
c'est extraire les heures et les minute de cette chaîne de caractère parce que c'est
ce dont on a besoin }
...
var
   ...
   // les employé qui sont pointable et qui ne sont pas encore pointer
   empPasPointer : array of string;
   { Les employés du jour qui sont pointé. C'est intéressant si vous fermez
   l'application après avoir pointé un certain nombre d'employés, à la reouverture, on
   va récupérer ceux sont pointer pour ne pas faire deux fois la même action. }
   empPointerFdp : array of string;
   { Les employé qui ont fait des heures sup. Même cas de figure que "empPointerFdp". }
   empPointerDHS : array of string;
   { Le tableau qui va contenir l'ensemble des employés à afficher à l'écran. }
   LesEmployeFdp : array of TemployeMaquetteFdp;
   { c'est pour avoir les l'ensemble des informations d'un employé lorsqu'on le pointe. }
   ...
   { nbEmpPointerFdp : le nombre d'employé qui ont été pointé; nbEmpPointerDHS :
   le nombre d'employés qui ont été pointer comme faisons des heures sup. }
   nbEmpPointerFdp, nbEmpPointerDHS : integer;
   { nbEleRecupFdpPasPointer : qu'on nous recupérons les informations d'un employé dans
   la base de données, nous recevons un certain nombre d'élément par exemple : nom,
   prenom... Cette variable compte le nombre de tous ces éléments. C'est la même logique
...
begin
// Pour faire varier les élément dans le tableau empPointerFdp
k := 0;
    ...
    For i := 0 to Length(LesEmployeFdp)-1 do
      begin
          ...
          if Length(empPointerDHS) <> 0 then
           begin
           // Pour faire varier les élément dans le tableau empPointerDHS
              m := 0;
              // Pour passer d'un employé en heures sup à l'autre.
              n := nbEleRecupDHS;
